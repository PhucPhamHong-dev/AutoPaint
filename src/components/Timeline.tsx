              ...
              >
                ...
                <div className="w-full md:w-1/2 px-0 md:px-12">
                  <Card className={`p-8 hover:scale-[1.02] transition-transform border-slate-200 ${i % 2 === 0 ? 'md:text-right' : 'md:text-left'}`}>
                    ...
                    <div
                      className={`mt-4 inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                        isReady
                          ? 'bg-amber-500/10 border border-amber-500/20 text-amber-600'
                          : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-600'
                      } ${i % 2 === 0 ? 'md:flex-row-reverse' : ''}`}
                    >
                      <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isReady ? 'bg-amber-500' : 'bg-emerald-500'}`} />
                      {m.status}
                    </div>
                  </Card>
                </div>
